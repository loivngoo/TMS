...
function getChartTotalDepartment() {
    const totalClient = Cookies.get('totalClient');

    if (totalClient) {
        return parseInt(totalClient);
    }
...
const state = {
    ...
    totalClient: getChartTotalDepartment(),
...
const mutations = {
    ...
    SET_TOTAL_DEPARTMENT: (state, totalClient) => {
        state.totalClient = totalClient;
        Cookies.set('totalClient', totalClient);
    },
...
const actions = {
    ...
    setTotalDepartment({ commit }, totalClient) {
        commit('SET_TOTAL_DEPARTMENT', totalClient);
    },
